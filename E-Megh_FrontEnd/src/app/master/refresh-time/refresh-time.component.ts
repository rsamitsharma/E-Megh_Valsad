...
import { RefreshTimeDialogComponent } from "./refresh-time-dialog/refresh-time-dialog.component";
...
@Component({
  selector: "app-refresh-time",
  templateUrl: "./refresh-time.component.html",
  styleUrls: ["./refresh-time.component.scss"],
})
export class RefreshTimeComponent extends UnSubscriber implements OnInit {
  constructor(private dialog: MatDialog, private _MasterService: MasterService, public _GlobalService: GlobalService) {
    super();
  }

  ngOnInit() {
    ({ DEFAULT_COLUMNS: this.displayedColumns, DEFAULT_TABLE_COLUMNS: this.defaultTableColumns } = this._GlobalService.tableColumnReformater(DEFAULT_COLUMNS.REFRESHTIME, DEFAULT_TABLE_COLUMNS.REFRESHTIME));
  ...
  GetRefreshTimeData() {
    this.anotherSubscription = this._MasterService.GetRefreshTimeMaster(0).subscribe((res: any) => {
      const { Table } = res;
      console.log(res);
  ...
  openDialog(id?: number) {
    ...
    const dialogRef = this.dialog.open(RefreshTimeDialogComponent, dialogConfig);
